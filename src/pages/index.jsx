...
function Rig() {
  return useFrame((state, delta) => {
    easing.damp3(state.camera.position, [0 + state.mouse.x / 4, 0.75 + state.mouse.y / 4, 2.5], 0.2, delta)
  })
}
...
export default function Page(props) {
  ...
  return (
    <><div>{(props.skin && props.visible) && names.map((nm, i) => {
      ...
      <div>{props.visible && Object.keys(skins).map((nm, i) => {
...
Page.canvas = (props) =>
  <><ambientLight intensity={0} /><directionalLight intensity={0} position={[-5, 5, 5]} castShadow shadow-mapSize={1024} /><group position={[0, -1, 0]}>
